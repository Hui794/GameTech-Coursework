...
public:
	...
	virtual void OnInitializeScene() override
	{
		...
		PhysicsEngine::Instance()->SetGravity(Vector3(0.0f, -9.8f, 0.0f));
		...
		ball = BuildSphereObject(
			"",						// Optional: Name
			Vector3(1.f, 20.f, 1.f),					// Position
			1.f,				// Half-Dimensions
			true,					// Physics Enabled?
			1.f,					// Physical Mass (must have physics enabled)
			true,					// Physically Collidable (has collision shape)
			true,					// Dragable by user?
			Vector4(1.f, 1.f, 1.f, 1.f));					// Render color
		PhysicsEngine::Instance()->SetGravity(Vector3(0.0, -9.81, 0.0f));
		this->AddGameObject(ball);
		cudaParticleProg->setParticles(ball);
