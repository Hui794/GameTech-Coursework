...
__global__
void CollideParticles(float baumgarte_factor, uint num_particles, Particle* particles, Particle* out_particles, uint* grid_cell_start, uint* grid_cell_end, float3 pos, float3 vel)
{
	...
	float3 ab = pos - p._pos;
	float lengthSq = dot(ab, ab);

	const float diameterSq = (1.0f - PARTICLE_RADIUS) * (1.0f - PARTICLE_RADIUS);
	if (lengthSq < diameterSq)
	{
		//We have a collision!
		float len = sqrtf(lengthSq);
		...
		//Direct normal collision (no friction/shear)
		float abnVel = dot(vel - p._vel, abn);
		float jn = -(abnVel * (1.f + COLLISION_ELASTICITY));
...
void CudaCollidingParticles::UpdateParticles(float dt)
{
	...
	for (int i = 0; i < 10; ++i)
	{
		const float world_dim = PARTICLE_GRID_SIZE * PARTICLE_GRID_CELL_SIZE;
		const float3 world_offset = make_float3(world_dim * 0.5f, 0.0f, world_dim * 0.5f);
		float3 pos = make_float3(ball->Physics()->GetPosition().x, ball->Physics()->GetPosition().y, ball->Physics()->GetPosition().z);
		pos += world_offset;
		float3 vel = make_float3(ball->Physics()->GetLinearVelocity().x, ball->Physics()->GetLinearVelocity().y, ball->Physics()->GetLinearVelocity().z);
		CollideParticles << < grid, block >> >(baumgarte_factor, num_particles, particles_ping, particles_pong, grid_cell_start, grid_cell_end, pos, vel);
