...
void Initialize()
{
	...
	SceneManager::Instance()->EnqueueScene(new TestScene("GameTech #1 - Shooting Dragon"));
	SceneManager::Instance()->EnqueueScene(new EmptyScene("GameTech #2 - BoolPool"));
	SceneManager::Instance()->EnqueueScene(new SoftBody("GameTech #3 - SoftBody"));
}
...
void PrintStatusEntries()
{
	...
	if (draw_debug)
	{
		...
		NCLDebug::AddStatusEntry(status_color_debug, "Manifolds         : %s [M] - Tut 5+", (drawFlags & DEBUGDRAW_FLAGS_MANIFOLD) ? "Enabled " : "Disabled");
	...
	NCLDebug::AddStatusEntry(status_colour, "     FPS: %5.2f  (Press G for %s info)", 1000.f / timer_total.GetAvg(), draw_performance ? "less" : "more");
