	...
	if (physicsNodes.size() > 0)
	{
		for (size_t i = 0; i < physicsNodes.size() - 1; ++i)
		{
			for (size_t j = i + 1; j < physicsNodes.size(); ++j)
			{
				...
				/*				if (pnodeA->GetParent()->Render() == NULL || pnodeB->GetParent()->Render() == NULL) {
				cout << "Non-parent Physical Node detected!" << endl;
				}*/

				if (pnodeA->GetParent() == NULL || pnodeB->GetParent() == NULL) {
					CollisionPair cp;
					cp.pObjectA = pnodeA;
					cp.pObjectB = pnodeB;
				...
				if (pnodeA->GetCollisionShape() != NULL && pnodeB->GetCollisionShape() != NULL && pnodeA->GetParent() != NULL && pnodeB->GetParent() != NULL)
				{
					float radiusA = pnodeA->GetParent()->Render()->GetBoundingRadius();
					float radiusB = pnodeB->GetParent()->Render()->GetBoundingRadius();
					Vector3 ab = pnodeA->GetPosition() - pnodeB->GetPosition();
