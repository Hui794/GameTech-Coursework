...
public:
	...
	void AddGameObject(GameObject* game_object)
	{
		if (game_object)
		{
			...
			if (game_object->physicsNode2) PhysicsEngine::Instance()->AddPhysicsObject(game_object->physicsNode2);
			if (game_object->PhysicsNodes.empty() == false) {
				for (int i = 0; i < game_object->PhysicsNodes.size(); ++i) {
					PhysicsEngine::Instance()->AddPhysicsObject(game_object->PhysicsNodes[i]);
				}
			}
		}
	}
	...
	void RemoveGameObject(GameObject* game_object)
	{
		if (game_object && game_object->scene == this)
		{
			...
			if (game_object->physicsNode2) PhysicsEngine::Instance()->RemovePhysicsObject(game_object->physicsNode2);
